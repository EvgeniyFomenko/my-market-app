...
public class ItemControllerTest {
    ...
    public void getItems(){
        Mockito.when(itemService.findItemsByTitle(null, PagableUtil.getPageable(1,5, VariableSort.NO.getFullName()))).thenReturn(Mono.just(new PageImpl<>(List.of(item))));
    ...
    public void getItemById(){
        Mockito.when(itemService.findById(item.getId())).thenReturn(Mono.just(item));
        webTestClient.get().uri("/items/" + item.getId().toString())
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("text/html")
                .expectBody(String.class).consumeWith(response -> {
